            ...
            <div className="movie-grid">
                ...
                <div className="infos-grid">
                    ...
                    <div style={{marginTop: '20px'}}>
                        <span className="main-cast">Box Office</span>
                        <div style={{marginTop: '10px', fontSize: '1.1rem'}}>
                            {details.revenue && details.revenue > 0 ? (
                                <>
                                    <div>Actual Revenue: ${details.revenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div>
                                    {movieStore.estimatedRevenue && (
                                        <div style={{color: '#01d277', marginTop: '5px'}}>
                                            Predicted Revenue: ${movieStore.estimatedRevenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                                        </div>
                                    )}
                                </>
                            ) : movieStore.estimatedRevenue ? (
                                <div style={{color: '#01d277'}}>
                                    Expected Revenue: ${movieStore.estimatedRevenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                                </div>
                ...
                <div className="details-grid">
                    ...
                    <div className="details-container">
                        <div className="details-title">Box Office</div>
                        <div className="details-value">
                            {details.revenue && details.revenue > 0 ? (
                                <>
                                    <div>Actual Revenue: ${details.revenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div>
                                    {movieStore.estimatedRevenue && (
                                        <div style={{fontSize: '0.9em', color: '#01d277', marginTop: '5px'}}>
                                            FilmBrain Prediction: ${movieStore.estimatedRevenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                                        </div>
                                    )}
                                </>
                            ) : movieStore.estimatedRevenue ? (
                                <div style={{color: '#01d277'}}>
                                    FilmBrain Predicted Revenue: ${movieStore.estimatedRevenue.toLocaleString('en-US', { maximumFractionDigits: 0 })}
                                </div>
                            ) : "-"}
                        </div>
                    </div>
